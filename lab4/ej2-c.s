...
reset:
    ...
    // El valor del semiperiodo para 24hs
    LDR R0,=86400
    STR R0,[R1,#MR3]    //Match provoca una interrupcion cada 86400 llamadas del prescaler

    // El valor del semiperiodo para alarma 2
    LDR R0,=60          //Alarma2 = 00:01 
    STR R0,[R1,#MR2]    //Match provoca una interrupcion cada 60 llamadas del prescaler

    // El valor del semiperiodo para alarma 3
    LDR R0,=180         //Alarma1 = 00:03
    STR R0,[R1,#MR1]    //Match provoca una interrupcion cada 180 llamadas del prescaler

    // El registro de match 3 provoca reset del contador
    MOV R0,#(MR3R | MR3I)   //Provoca reset e interrupcion que lleva a rutina
    STR R0,[R1,#MCR]
    // El registro de match 3 provoca reset del contador
    MOV R0,#MR1I   //Provoca reset e interrupcion que lleva a rutina
    STR R0,[R1,#MCR]
    // El registro de match 3 provoca reset del contador
    MOV R0,#MR2I   //Provoca reset e interrupcion que lleva a rutina
    STR R0,[R1,#MCR]
...
actualizar_reloj:
    ...
    LDR R4,=60    
    PUSH {R0}
    ADD R3,R0,#0        //R3 = valor contador
    BL modulo           //Devuelve en R3 el valor de los segundos
    POP {R0}
    BL dec_bcd          //Convierto el valor de R3 en decimal a BCD para guardar unidad y decena separada
    STRB R3,[R2]
    STRB R5,[R2,#1]
    //Contador minutos
    LDR R4,=60        
    UDIV R3,R0,R4       //R3 = division entera entre el contador y 60 segundos (1min)
    BL dec_bcd          //Decimal a BCD, R3 = unidad, R5 = decena
    STRB R3,[R2,#2]
    STRB R5,[R2,#3]
    //Contador horas
    LDR R4,=3600
    UDIV R3,R0,R4       //R3 = division entera entre el contador y 3600 segundos (1hs)
    BL dec_bcd          //Decimal a BCD, R3 = unidad, R5 = decena
    STRB R3,[R2,#4]
    STRB R5,[R2,#5]   
    POP {PC}
...
config_led:
    PUSH {LR}
    LDR R1,=SCU_BASE
    // Se configuran los pines del led 3 como gpio sin pull-up
    MOV R0,#(SCU_MODE_INACT | SCU_MODE_INBUFF_EN | SCU_MODE_ZIF_DIS | SCU_MODE_FUNC0)
    STR R0,[R1,#(LED_3_PORT << 7 | LED_3_PIN << 2)]
    // Se apagan los bits gpio correspondientes al led3
    LDR R0,=LED_3_MASK
    STR R0,[R1,#LED_3_OFFSET]
    // Se configuran los bits gpio del led3 como salida
    LDR R0,=LED_3_MASK
    STR R0,[R1,#LED_3_OFFSET]
    POP {PC}        // Se retorna al programa principal
    .pool           // Se almacenan las constantes de codigo
